...
(function () {
  ...
  window.getSimilarArray = function (colElementArray) {
    for (var i = 0; i < colElementArray; i++) {
      ...
      var locationX = Math.floor(Math.random() * (window.initialData.MAX_LOCATION_X - window.initialData.MIN_LOCATION_X) + window.initialData.MIN_LOCATION_X);
      var locationY = Math.floor(Math.random() * (window.initialData.MAX_LOCATION_Y - window.initialData.MIN_LOCATION_Y) + window.initialData.MIN_LOCATION_Y);
      var element = i + 1;
      similarAds[i] = {
        author: {
        ...
        offer: {
          ...
          price: Math.floor(Math.random() * (window.initialData.MAX_PRICE - window.initialData.MIN_PRICE) + window.initialData.MIN_PRICE),
          type: adType[Math.floor(Math.random() * (adType.length - 1))],
          rooms: Math.floor(Math.random() * (window.initialData.MAX_ROOMS - window.initialData.MIN_ROOMS) + window.initialData.MIN_ROOMS),
          guests: Math.floor(Math.random() * (window.initialData.MAX_GUESTS - window.initialData.MIN_GUESTS) + window.initialData.MIN_GUESTS),
